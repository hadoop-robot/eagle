    ...
    public void truncateJobBefore(JobConstants.ResourceType type, String date) {
        ...
        try {
            ...
            if (curator.checkExists().forPath(path) != null) {
                ...
                List<String> jobIDList = curator.getChildren().forPath(path);
                for (String jobID : jobIDList) {
                    if (!jobID.startsWith("job_")) {
                        continue; // skip lock node
                    }
                    String jobPath = path + "/" + jobID;
                    long createTime = getTimestampFromDate(new String(curator.getData().forPath(jobPath), StandardCharsets.UTF_8));
                    if (createTime < thresholdTime) {
                        LOG.info("Going to truncate job: " + jobPath);
                        curator.delete().deletingChildrenIfNeeded().forPath(jobPath);
                    }
                }
            } else {
                LOG.info("Currently processed job list is empty, type: " + type.name());
