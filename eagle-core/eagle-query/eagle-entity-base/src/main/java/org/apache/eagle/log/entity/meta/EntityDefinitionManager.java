...
public class EntityDefinitionManager {
    ...
    public static void registerEntity(String serviceName, EntityDefinition entityDef) {
        ...
        if (entityServiceMap.containsKey(serviceName)) {
            ...
            throw new IllegalArgumentException("Service " + serviceName + " has already been registered by "
                + existing.getClass().getName() + ", so class " + entityDef.getClass() + " can NOT be registered");
        }
        ...
        if (entityPrefixMap.containsKey(entityDef.getTable())) {
            ...
            if (entityHashMap.containsKey(entityPrefixHashcode) && (!entityDef.equals(entityHashMap.get(entityPrefixHashcode)))) {
                throw new IllegalArgumentException("Failed to register entity " + entityDef.getClass().getName()
                    ...
                    + " has already been registered by entity service " + entityHashMap.get(entityPrefixHashcode).getService());
            }
            final IndexDefinition[] indexes = entityDef.getIndexes();
            if (indexes != null) {
                for (IndexDefinition index : indexes) {
                    ...
                    if (entityHashMap.containsKey(indexPrefixHashcode)) {
                        throw new IllegalArgumentException("Failed to register entity " + entityDef.getClass().getName()
                            + ", because of the prefix hash code conflict! The index prefix " + index.getIndexPrefix()
                            + " has already been registered by entity " + entityHashMap.get(indexPrefixHashcode).getService());
                    }
                    ...
                    if (indexHashMap != null && indexHashMap.containsKey(indexPrefixHashcode) && (!index.equals(indexHashMap.get(indexPrefixHashcode)))) {
                        throw new IllegalArgumentException("Failed to register entity " + entityDef.getClass().getName()
                            + ", because of the prefix hash code conflict! The index prefix " + index.getIndexPrefix()
                            + " has already been registered by entity " + indexHashMap.get(indexPrefixHashcode).getEntityDefinition().getService());
                    }
                }
            }
        }
     ...
     *
     * @param clazz TaggedLogAPIEntity class type
     * @return whether the entity is time-series, if yes, return true, else return false.
     */
