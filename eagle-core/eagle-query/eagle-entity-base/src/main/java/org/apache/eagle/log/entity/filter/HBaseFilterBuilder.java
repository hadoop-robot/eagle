...
public class HBaseFilterBuilder {
     ...
     * Get the set of filter files.
     *
     * @return the set of filter files
     */
    ...
    private boolean isTag(String field) {
        return entityDefinition.isTag(field);
    }
    ...
    public List<String[]> getPartitionValues() {
        final String[] partitions = entityDefinition.getPartitions();
        ...
        for (ANDExpression andExpr : orExpr.getANDExprList()) {
            ...
            for (AtomicExpression ae : andExpr.getAtomicExprList()) {
                ...
                if (ae.getKeyType() == TokenType.ID) {
                    ...
                    if (entityDefinition.isPartitionTag(fieldName) && ComparisonOperator.EQUAL.equals(ae.getOp())) {
            ...
            for (AtomicExpression ae : andExpr.getAtomicExprList()) {
                ...
                if (keyType == TokenType.ID && isTag(fieldName)) {
                    if ((ComparisonOperator.EQUAL.equals(op) || ComparisonOperator.IS.equals(op))
                        ...
                        throw new IllegalArgumentException("Don't support operation: " + op + " on tag field: "
                            + fieldName + ", avaliable options: =, =!, =~, !=~, in, not in, contains, not contains");
                    }
                } else {
                    qualifierFilters.add(entry);
                }
    ...
    /**
     * charset is used to decode the byte array, in hbase server, RegexStringComparator uses the same
     ...
     * </H2>
     *
     * @param qualifierFilters qualifierFilters
    ...
    protected FilterList buildQualifierFilter(List<QualifierFilterEntity> qualifierFilters) {
        ...
        for (QualifierFilterEntity entry : qualifierFilters) {
            ...
            if (!isTag(entry.getKey())) {
                Qualifier qualifier = entityDefinition.getDisplayNameMap().get(entry.getKey());
            ...
            if (ComparisonOperator.IN.equals(entry.getOp())
                ...
                if (TokenType.NULL.equals(entry.getValueType())) {
                    if (ComparisonOperator.EQUAL.equals(entry.getOp())
                        ...
                        throw new IllegalArgumentException("Operation: " + entry.getOp() + " with NULL is not supported yet: "
                            + entry.toString() + ", avaliable options: [=, !=, is, is not] null|NULL");
                    }
                    ...
                    RegexStringComparator regexComparator = new RegexStringComparator(buildQualifierRegex(entry.getValue()));
                    regexComparator.setCharset(charset);
                    comparable = regexComparator;
                } else {
                    Class type = EntityQualifierUtils.getType(entityDefinition, entry.getKey());
                    ...
                    if (!EagleConfigFactory.load().isCoprocessorEnabled() || type == null || TypedByteArrayComparator.get(type) == null) {
                        comparable = new BinaryComparator(EntityQualifierUtils.toBytes(entityDefinition, entry.getKey(), entry.getValue()));
                    } else {
                        comparable = new TypedByteArrayComparator(EntityQualifierUtils.toBytes(entityDefinition, entry.getKey(), entry.getValue()), type);
                    }
                }

                SingleColumnValueFilter filter =
                    new SingleColumnValueFilter(entityDefinition.getColumnFamily().getBytes(), qualifierName.getBytes(), convertToHBaseCompareOp(entry.getOp()), comparable);
    ...
    private Filter buildExpressionBasedFilter(QualifierFilterEntity entry) {
        BooleanExpressionComparator expressionComparator = new BooleanExpressionComparator(entry, entityDefinition);
     ...
     *
     * @param entry QualifierFilterEntity
     * @return Filter
     */
    private Filter buildListQualifierFilter(QualifierFilterEntity entry) {
        ...
        if (!entityDefinition.isTag(entry.getKey())) {
            qualifierName = entityDefinition.getDisplayNameMap().get(entry.getKey()).getQualifierName();
        }
        ...
        //  Class qualifierType = EntityQualifierHelper.getType(entityDefinition,fieldName);
        ...
        //          return new SingleColumnValueFilter(entityDefinition.getColumnFamily().getBytes(), qualifierName.getBytes(),
        ...
        while (it.hasNext()) {
            String value = it.next();
            BinaryComparator comparator = new BinaryComparator(EntityQualifierUtils.toBytes(entityDefinition, fieldName, value));
            SingleColumnValueFilter filter =
                new SingleColumnValueFilter(entityDefinition.getColumnFamily().getBytes(), qualifierName.getBytes(), convertToHBaseCompareOp(entry.getOp()), comparator);
            filter.setFilterIfMissing(filterIfMissing);
     ...
     *
     * @param qualifierValue qualifierValue
    ...
    protected String buildTagFilterRegex(Map<String, List<String>> tags) {
        ...
        final int numOfPartitionFields = (entityDefinition.getPartitions() == null) ? 0 : entityDefinition.getPartitions().length;
        for (Map.Entry<String, List<String>> entry : tags.entrySet()) {
            String tagName = entry.getKey();
            // Ignore tag if the tag is one of partition fields
            if (entityDefinition.isPartitionTag(tagName)) {
                continue;
            }
