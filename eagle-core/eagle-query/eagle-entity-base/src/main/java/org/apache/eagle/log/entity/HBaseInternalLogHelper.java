                    ...
                    try {
                        ...
                        if (isMetricEntity && isRequiringValue && doubleMap.get(GenericMetricEntity.VALUE_FIELD) != null
                            ...
                            if (metricValueArray != null) {
                                ...
                                Map<String, Double> clonedDoubleMap = new HashMap<String, Double>(doubleMap);
                                clonedDoubleMap.remove(entry.getKey());
                                for (int i = 0; i < resultBucket.length; i++) {
                                    clonedDoubleMap.put(GenericMetricEntity.VALUE_FIELD, metricValueArray[i]);
                                    resultBucket[i] = parser.eval(clonedDoubleMap);
                                }
